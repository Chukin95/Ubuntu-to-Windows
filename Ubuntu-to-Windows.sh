...
if [ $link1_status = "200" ] ; then
	...
	if [ "$actual_iso_sha256" != "$expected_iso_sha256" ]; then
		echo "Error: el SHA-256 de la ISO no coincide con el valor esperado."
		echo "Esperado: $expected_iso_sha256"
		echo "Obtenido:  $actual_iso_sha256"
		exit 1
	fi
